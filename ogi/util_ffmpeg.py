...
def run_ffmpeg(args: List[str]) -> bool:
    commands = ['ffmpeg', '-hide_banner', '-hwaccel', 'auto', '-y', '-loglevel', ogi.globals.log_level]
...
def cut_video(original_video: str, cut_video: str, start_frame: int, end_frame: int):
    ...
    run_ffmpeg(['-ss',  str(start_time), '-i', original_video, '-c:v', ogi.globals.video_encoder, '-c:a', 'aac', '-frames:v', str(num_frames), cut_video])
...
def extract_frames(target_path : str, trim_frame_start, trim_frame_end, fps : float) -> bool:
    ...
    commands.extend(['-vsync', '0', os.path.join(temp_directory_path, '%04d.' + ogi.globals.CFG.output_image_format)])
...
def create_video(target_path: str, dest_filename: str, fps: float = 24.0, temp_directory_path: str = None) -> None:
    if temp_directory_path is None:
        temp_directory_path = util.get_temp_directory_path(target_path)
    run_ffmpeg(['-r', str(fps), '-i', os.path.join(temp_directory_path, f'%04d.{ogi.globals.CFG.output_image_format}'), '-c:v', ogi.globals.video_encoder, '-crf', str(ogi.globals.video_quality), '-pix_fmt', 'yuv420p', '-vf', 'colorspace=bt709:iall=bt601-6-625:fast=1', '-y', dest_filename])
    return dest_filename


def create_gif_from_video(video_path: str, gif_path):
    from ogi.capturer import get_video_frame
