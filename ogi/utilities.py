...
import ogi.template_parser as template_parser
...
def sort_rename_frames(path: str):
    ...
    for i in range(len(filenames)):
        ...
        newidx = i+1
        new_filename = os.path.join(path, f"{newidx:04d}." + ogi.globals.CFG.output_image_format)
        os.rename(of, new_filename)        


def get_temp_frame_paths(target_path: str) -> List[str]:
    temp_directory_path = get_temp_directory_path(target_path)
    return glob.glob((os.path.join(glob.escape(temp_directory_path), f'*.{ogi.globals.CFG.output_image_format}')))
...
def replace_template(file_path: str, index: int = 0):
    ...
    template = ogi.globals.CFG.output_template
    replaced_filename = template_parser.parse(template, {
        'index': str(index),
        'file': fn
    })

    return os.path.join(ogi.globals.output_path, f'{replaced_filename}{ext}')
...
def clean_temp(target_path: str) -> None:
    ...
    if not ogi.globals.keep_frames and os.path.isdir(temp_directory_path):
...
def get_device() -> str:
    if len(ogi.globals.execution_providers) < 1:
        ogi.globals.execution_providers = ['CPUExecutionProvider']

    prov = ogi.globals.execution_providers[0]
...
def prepare_for_batch(target_files):
    ...
    tempfolder = os.path.join(tempfile.gettempdir(), "ogitmp")
