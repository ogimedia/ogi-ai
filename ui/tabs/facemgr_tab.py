...
def on_fb_files_changed(inputfiles, progress=gr.Progress()):
    ...
    for f in inputfiles:
        ...
        if util.has_image_extension(source_path):
            ...
            SELECTION_FACES_DATA = extract_face_images(ogi.globals.source_path,  (False, 0), 0.5)
            for f in SELECTION_FACES_DATA:
                image = f[1]
                images.append(image)
                thumbs.append(util.convert_to_gradio(image))
...
def on_update_clicked():
    ...
    for index,img in enumerate(images):
        filename = os.path.join(ogi.globals.output_path, f'{index}.png')
    ...
    finalzip = os.path.join(ogi.globals.output_path, 'faceset.fsz')        
