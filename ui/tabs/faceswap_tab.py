...
from ogi.capturer import get_video_frame, get_video_frame_total, get_image_frame
...
def faceswap_tab():
    ...
    with gr.Tab("🎭 Face Swap"):
        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column(scale=1):
                ...
                with gr.Box():
                    ogi.globals.skip_audio = gr.Checkbox(label="Skip audio", value=False)
                    ogi.globals.keep_frames = gr.Checkbox(label="Keep Frames (relevant only when extracting frames)", value=False)
                    ogi.globals.wait_after_extraction = gr.Checkbox(label="Wait for user key press before creating video ", value=False)
            with gr.Column(scale=1):
        ...
        with gr.Row(variant='panel'):
            with gr.Column():
                ...
                gr.Button("👀 Open Output Folder", size='sm').click(fn=lambda: util.open_folder(ogi.globals.output_path))
            with gr.Column():
    ...
    start_event = bt_start.click(fn=start_swap, 
        inputs=[ui.globals.ui_selected_enhancer, selected_face_detection, ogi.globals.keep_frames,
                    ogi.globals.skip_audio, max_face_distance, ui.globals.ui_blend_ratio, chk_useclip, clip_text,video_swapping_method, no_face_action],
...
def on_mask_top_changed(mask_offset):
    ...
    if len(ogi.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        ogi.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets[0] = mask_offset

def on_mask_bottom_changed(mask_offset):
    global SELECTED_INPUT_FACE_INDEX

    if len(ogi.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        ogi.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets[1] = mask_offset
...
def on_srcfile_changed(srcfiles, progress=gr.Progress()):
    from ogi.face_util import norm_crop2
    ...
    for f in srcfiles:    
        ...
        if source_path.lower().endswith('fsz'):
            ...
            if len(face_set.faces) > 0:
                ...
                ogi.globals.INPUT_FACESETS.append(face_set)
                                        
        elif util.has_image_extension(source_path):
            progress(0, desc="Retrieving faces from image", )      
            ogi.globals.source_path = source_path
            SELECTION_FACES_DATA = extract_face_images(ogi.globals.source_path,  (False, 0))
...
def remove_selected_input_face():
    ...
    if len(ogi.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        f = ogi.globals.INPUT_FACESETS.pop(SELECTED_INPUT_FACE_INDEX)
        del f
...
def remove_selected_target_face():
    if len(ogi.globals.TARGET_FACES) > SELECTED_TARGET_FACE_INDEX:
        f = ogi.globals.TARGET_FACES.pop(SELECTED_TARGET_FACE_INDEX)
        del f
...
def on_use_face_from_selected(files, frame_num):
    ...
    ogi.globals.target_path = files[selected_preview_index].name
    if util.is_image(ogi.globals.target_path) and not ogi.globals.target_path.lower().endswith(('gif')):
        SELECTION_FACES_DATA = extract_face_images(ogi.globals.target_path,  (False, 0))
        if len(SELECTION_FACES_DATA) > 0:
            ...
            ogi.globals.target_path = None
                
    elif util.is_video(ogi.globals.target_path) or ogi.globals.target_path.lower().endswith(('gif')):
        selected_frame = frame_num
        SELECTION_FACES_DATA = extract_face_images(ogi.globals.target_path, (True, selected_frame))
        if len(SELECTION_FACES_DATA) > 0:
            for f in SELECTION_FACES_DATA:
                image = util.convert_to_gradio(f[1])
                thumbs.append(image)
    ...
    if len(thumbs) == 1:
        ogi.globals.TARGET_FACES.append(SELECTION_FACES_DATA[0][0])
...
def on_selected_face():
    ...
    if IS_INPUT:
        ...
        ogi.globals.TARGET_FACES.append(fd[0])
...
def on_preview_frame_changed(frame_num, files, fake_preview, enhancer, detection, face_distance, blend_ratio, use_clip, clip_text, no_face_action):
    ...
    from ogi.core import live_swap

    mask_offsets = (0,0)
    if len(ogi.globals.INPUT_FACESETS) > SELECTED_INPUT_FACE_INDEX:
        if not hasattr(ogi.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0], 'mask_offsets'):
            ogi.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets = mask_offsets
        mask_offsets = ogi.globals.INPUT_FACESETS[SELECTED_INPUT_FACE_INDEX].faces[0].mask_offsets
    ...
    if not fake_preview or len(ogi.globals.INPUT_FACESETS) < 1:
        return util.convert_to_gradio(current_frame), mask_offsets[0], mask_offsets[1]

    ogi.globals.face_swap_mode = translate_swap_mode(detection)
    ogi.globals.selected_enhancer = enhancer
    ogi.globals.distance_threshold = face_distance
    ogi.globals.blend_ratio = blend_ratio
    ogi.globals.no_face_action = index_of_no_face_action(no_face_action)

    if use_clip and clip_text is None or len(clip_text) < 1:
        use_clip = False

    ogi.globals.execution_threads = ogi.globals.CFG.max_threads
    current_frame = live_swap(current_frame, ogi.globals.face_swap_mode, use_clip, clip_text, SELECTED_INPUT_FACE_INDEX)
    if current_frame is None:
        return None, mask_offsets[0], mask_offsets[1] 
    return util.convert_to_gradio(current_frame), mask_offsets[0], mask_offsets[1]
...
def on_preview_mask(frame_num, files, clip_text):
    from ogi.core import preview_mask
...
def start_swap( enhancer, detection, keep_frames, skip_audio, face_distance, blend_ratio,
    ...
    ogi.globals.distance_threshold = face_distance
    ogi.globals.blend_ratio = blend_ratio
    ogi.globals.keep_frames = keep_frames
    ogi.globals.skip_audio = skip_audio
    ogi.globals.face_swap_mode = translate_swap_mode(detection)
    ogi.globals.no_face_action = index_of_no_face_action(no_face_action)
    if use_clip and clip_text is None or len(clip_text) < 1:
        use_clip = False
    
    if ogi.globals.face_swap_mode == 'selected':
        if len(ogi.globals.TARGET_FACES) < 1:
    ...
    is_processing = False
    outdir = pathlib.Path(ogi.globals.output_path)
