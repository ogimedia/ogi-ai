...
def on_cut_video(files, cut_start_frame, cut_end_frame):
    ...
    for tf in files:
        f = tf.name
        destfile = util.get_destfilename_from_path(f, ogi.globals.output_path, '_cut')
...
def on_join_videos(files, chk_encode):
    ...
    destfile = util.get_destfilename_from_path(filenames[0], ogi.globals.output_path, '_join')
...
def on_extras_create_video(images_path,fps, create_gif):
    ...
    destfilename = os.path.join(ogi.globals.output_path, "img2video." + ogi.globals.CFG.output_video_format)
